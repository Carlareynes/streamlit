...
df=  pd.read_csv("airbnb.csv")  # Ensure the file is in the same directory

# ---- SIDEBAR ----
st.sidebar.header("Filter Options")
neighborhood = st.sidebar.multiselect("Select Neighborhood", df["neighbourhood"].unique(), default=df["neighbourhood"].unique())
...
with tab1:
    ...
    # Relationship between listing type and number of people
    fig2 = px.box(filtered_data, x="room_type", y="accommodates", title="People Capacity by Listing Type")
...
user_listing_type = st.sidebar.selectbox("Select Listing Type", df["room_type"].unique())
